...
export default function DrinkFeed() {
  ...
  {drinks.map((drink) => (
    <div key={drink.id} className="relative aspect-[4/5] rounded-[2.5rem] overflow-hidden border border-white/5 shadow-2xl bg-[#141417]">
      ...
      {/* Barre d'infos supérieure centrée et étalée */}
      <div className="absolute top-4 left-4 right-4 flex items-center justify-between pointer-events-none">
        
        {/* 1. Pill Utilisateur (Gauche) */}
        <div className="flex items-center gap-2 bg-black/60 backdrop-blur-md p-1.5 pr-4 rounded-full border border-white/10 pointer-events-auto">
        ...
        {/* 2. Pill Boisson (Droite) */}
        <div className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 shadow-lg pointer-events-auto">
          <span className="text-[9px] font-black text-[#DFFF5E] uppercase tracking-[0.15em] block leading-none">
            {drink.drink_type}
          </span>
        </div>
